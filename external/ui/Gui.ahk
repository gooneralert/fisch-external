...
GetGui() {
    ...
    mg.Title := "Ardvark fisch | " FULL_VER
    mg.SetFont(, "Segoe UI")
    ...
    AccessabilityHeader:= mg.AddText("x10 y25 w400 h40 c" TextColor, "Accessability")
    ...
    StopAppraiseKey := mg.AddHotkey("x10 y120 w30 h20", SETTINGS["hotkeys"].Has("stop_appraise") ? SETTINGS["hotkeys"]["stop_appraise"] : "F2")
    StopAppraiseKey.OnEvent("Change", (ctrl, *) => UpdateHotkey("stop_appraise", ctrl))
    mg.AddText("x50 y119 w150 h20 c" TextColor, "Stop Appraising").SetFont("s11")
    mg.AddText("x50 y138 w250 h20 c646464", "Stops an active appraise cycle.")

    FixRbxKey := mg.AddHotkey("x10 y165 w30 h20", SETTINGS["hotkeys"]["fix_roblox"])
    FixRbxKey.OnEvent("Change", (ctrl, *) => UpdateHotkey("fix_roblox", ctrl))
    mg.AddText("x50 y164 w100 h20 c" TextColor, "Fix Roblox").SetFont("s11")
    mg.AddText("x50 y183 w250 h20 c646464", "Do this if XTernal cant read your rod")
    FixRbxHelpBtn := mg.AddText("x125 y167 w100 h20 c" Accent, "Learn More")
    ...
    ReloadKey := mg.AddHotkey("x10 y210 w30 h20", SETTINGS["hotkeys"]["reload"])
    ReloadKey.OnEvent("Change", (ctrl, *) => UpdateHotkey("reload", ctrl))
    mg.AddText("x50 y209 w100 h20 c" TextColor, "Reload").SetFont("s11")
    mg.AddText("x50 y228 w250 h20 c646464", "Change the hotkey with which you reload the macro.")

    AppearanceHeader := mg.AddText("x10 y260 w400 h40 c" TextColor, "Appearance")
    AppearanceHeader.SetFont("s15")
    border(mg, 10, 295, 380, 1)

    mg.AddText("x10 y302 w80 h20 c" TextColor, "Theme").SetFont("s10")
    ...
    ThemeDDL := mg.AddDDL("x260 y300 w110 h200", themeNames)
    ...
    mg.AddText("x10 y332 w100 h20 c" TextColor, "Accent color").SetFont("s10")
    AccentInput := mg.AddEdit("x260 y331 w80 h20", APPEARANCE["accent_color"])
    AccentSwatch := mg.AddText("x350 y331 w20 h20 +Border Background" APPEARANCE["accent_color"], "")

    mg.AddText("x10 y359 w100 h20 c" TextColor, "Background").SetFont("s10")
    BgInput := mg.AddEdit("x260 y358 w80 h20", APPEARANCE["bg_color"])
    BgSwatch := mg.AddText("x350 y358 w20 h20 +Border Background" APPEARANCE["bg_color"], "")

    mg.AddText("x10 y386 w100 h20 c" TextColor, "Text color").SetFont("s10")
    TextInput := mg.AddEdit("x260 y385 w80 h20", APPEARANCE["text_color"])
    TextSwatch := mg.AddText("x350 y385 w20 h20 +Border Background" APPEARANCE["text_color"], "")

    mg.AddText("x10 y413 w100 h20 c" TextColor, "Border color").SetFont("s10")
    BorderInput := mg.AddEdit("x260 y412 w80 h20", APPEARANCE["border_color"])
    BorderSwatch := mg.AddText("x350 y412 w20 h20 +Border Background" APPEARANCE["border_color"], "")
    appearanceFields := [
    ...
    ApplyAppearanceBtn := button(mg, "Apply", 290, 442, {
        w: 80,
        h: 25,
        bg: Accent
    })
    ...
    mg.AddText("x10 y445 w240 h20 c" SubColor, "Press Apply to save and reload.")
    ...
    MainTab.UseTab(5)
    mg.AddText("x10 y30 w300 h40 c" TextColor, "Ardvark fisch").SetFont("s15 bold")
    mg.AddText("x10 y60 w300 h40 c" TextColor, "Designed, developed && maintained by catz").SetFont("s10")
    mg.AddText("x10 y100 w380 h40 c" TextColor, "Thanks to myself (@catz) <3").SetFont("s10")
    ...
    CreditsDiscordLink.OnEvent("Click", (*) => Run("https://discord.gg/qN2trcyPT"))

    CreditsWebLink := mg.AddText("x10 y560 w200 h20 c" Accent, "Official Website")
    CreditsWebLink.SetFont("underline")
    CreditsWebLink.OnEvent("Click", (*) => Run("https://discord.gg/qN2trcyPT"))
